...
import { hasFinePointer, textureUrl } from "@/lib/motion";
import {
  createEmberMaterial,
  createPlaneMaterial,
  createSeatMaterial,
...
// photos right of the copy; phones transform these (see planeX / planeY).
...
const planeX = (x: number, mobile: boolean) => (mobile ? x * 0.35 - 1.0 : x);
const planeY = (floor: number, y: number, mobile: boolean) => floor * FLOOR + (mobile ? y * 0.5 + 3.4 : y);

/** Per-frame values shared by every object in the scene (never React state). */
type Sim = { t: number; camY: number; bend: number; lastP: number; mx: number; my: number; mobile: boolean };
...
function Rig({ journey, sim, onReady }: { journey: JourneyState; sim: Sim; onReady: () => void }) {
  ...
  useEffect(() => {
    sim.mobile = size.width < 760;
    cam.fov = sim.mobile ? 58 : 42;
    cam.updateProjectionMatrix();
  }, [cam, size.width, sim]);

  // Mouse parallax: mouse only, so phones never get it.
  useEffect(() => {
  ...
  useFrame((state, dt) => {
    ...
    cam.position.set(sim.mx * 1.2, sim.camY - sim.my * 0.6, 16);
    cam.lookAt(sim.mx * 0.4, sim.camY, 0);
...
function Plane({ spec, floor, sim }: { spec: Spec; floor: number; sim: Sim }) {
  ...
  const phone = useThree((s) => s.size.width < 760);
  ...
  const tex = useLazyTexture(textureUrl(src, phone), load);
  ...
  useFrame((_, dt) => {
    ...
    const tx = planeX(x, sim.mobile) + Math.sin(sim.t * 0.25 + z) * 0.08;
    m.position.x += (tx - m.position.x) * lerpAt60(0.05, dt);
    m.position.y = planeY(floor, y, sim.mobile);
  });

  return <mesh ref={mesh} geometry={geometry} material={material} position={[planeX(x, sim.mobile), planeY(floor, y, sim.mobile), z]} />;
}
...
function Seats({ sim, journey }: { sim: Sim; journey: JourneyState }) {
  ...
  const geometry = useMemo(() => {
    // Ten theatre rows of twelve with a centre aisle, rising away from the camera.
  ...
  useFrame(() => {
    ...
    u.uPx.value = dpr * (size.height / 900);
    u.uTime.value = sim.t;
...
function Embers({ sim }: { sim: Sim }) {
  const size = useThree((s) => s.size);
  const dpr = useThree((s) => s.viewport.dpr);
  const count = size.width < 760 ? 160 : 420;
...
export default function HomeScene({
  ...
  return (
    ...
    >
      ...
      <Wave sim={sim} />
      {LAYOUT.map((list, floor) => list.map((spec) => <Plane key={spec[0]} spec={spec} floor={floor} sim={sim} />))}
